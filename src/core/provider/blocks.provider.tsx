...
export const BlocksContext: Context<BlockContextProps> = React.createContext(
  {} as unknown as BlockContextProps
);

const patchBlocks = (
  oldBlocks: ExtendedBlock[],
  newBlocks: ExtendedBlock[]
) => {
  if (!newBlocks.length) {
    return [...oldBlocks];
  }

  let sliceIndex = oldBlocks.findIndex(
    block => block.height >= newBlocks[0].height
  );

  sliceIndex = sliceIndex < 0 ? oldBlocks.length : sliceIndex;

  return [...oldBlocks.slice(0, sliceIndex), ...newBlocks];
};

const retriveBlock = async (
  api: ApiRx,
  blockHash: string,
  height: number
): Promise<{
  ...
  const [block, events] = await firstValueFrom(
    zip(
  ...
  const extrinsics: ExtendedExtrinsic[] = block.block.extrinsics.map(
    (extrinsic, index) => {
...
const retriveBlocks = async (
  ...
  const blocks: NullableBlock[] = await Promise.all(
    new Array(endHeight - startHeight + 1).fill(1).map(async (_, i) => {
      try {
        const blockHash = await firstValueFrom(
          api.rpc.chain.getBlockHash(startHeight + i)
        );
  ...
  console.log(
    'Retrive Blocks From Node:',
    `${startHeight} -> ${endHeight}`,
    blocks.filter(block => !!block).map(b => b?.toHuman())
  );
...
const retriveLatestBlocks = async (
  api: ApiRx,
  startIndex = 0
): Promise<ExtendedBlock[]> => {
...
export const BlocksProvider = React.memo(
  ({ children }: { children: React.ReactNode }): React.ReactElement => {
    ...
    //   '0xfeacc106a2d55b61c2946f9670e2aa0cc062a25144b1ebf5cf321185928170a5',
    //   '',
    //   '',
    //   console.log
    // )
    ...
    const backward = useCallback(
      (height: number) => {
        ...
        (api as any).rpc.europa
          .backwardToHeight(height)
          .pipe(finalize(() => setBackwarding(false)))
          .subscribe(
            async () => {
    ...
    useEffect(() => {
      ...
      const newSub = from(retriveLatestBlocks(api))
        .pipe(
          tap(_blocks => {
            blocksRef.current = _blocks;
            setBlocks(_blocks);
