...
export const Message: FC<{
  ...
  message: AbiMessage;
  index: number;
}> = ({ contract, message, index }): ReactElement => {
  ...
  const isDisabled = useMemo(
    () => !gasLimit || !sender || !params.every(param => param.isValid),
    [gasLimit, sender, params]
  );
  ...
  useEffect(() => {
    ...
    try {
      const sub = contract.query[message.method](
        sender,
        { gasLimit: -1, value: 0 },
        ...
        if (result.isOk) {
          setGasLimit(
            new BN(gasConsumed.toString()).div(BN_MILLION).toNumber() + 1
          );
        }
      });
  ...
  const CallWithTrace = useCallback(async () => {
    const data = extractCallData(
      ...
      new BN(gasLimit).mul(BN_MILLION)
    );

    wsProvider
      .send('contractsExt_call', [
        {
          origin: sender,
      ...
      {expanded && (
        <div className='call'>
          ...
          <Exec style={{ marginTop: message.isMutating ? '0px' : '20px' }}>
            <Button disabled={isDisabled} type='primary' onClick={send}>
              {message.isMutating ? 'Execute' : 'Read'}
            </Button>
            <Button
              disabled={isDisabled}
              type='primary'
          ...
          {!message.isMutating && result && (
            <div className='result'>
              <Result>
                <span className='type'>
                  {message.returnType &&
                    encodeTypeDef(api.registry, message.returnType)}
                  :{' '}
                </span>
